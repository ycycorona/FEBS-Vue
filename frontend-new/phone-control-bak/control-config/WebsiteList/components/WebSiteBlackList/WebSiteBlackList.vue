<template>
  <a-spin :spinning="loading">
    <div class="website-black-list-tab">
      <div class="float-add-btn">
        <a-button
          ...
          @click="openCreate"
        >
          <a-icon type="plus" /><span style="margin-left: 3px;">添加网站黑名单</span>
        </a-button>
      </div>
      <!-- 表格区域 -->
      ...
      <a-table
        ref="website-black-list-tab-table"
      ...
      <CreateBlackWebsiteListPop
        :visible.sync="createBlackWebsiteListPopVisible"
        :is-edit.sync="isEdit"
        :edit-id.sync="editId"
        @close="handleCreateBlackWebsiteListClose"
        @success="handleCreateBlackWebsiteListSuccess"
      ></CreateBlackWebsiteListPop>
    </div>
  </a-spin>
...
import CreateBlackWebsiteListPop from './components/CreateBlackWebsiteListPop/CreateBlackWebsiteListPop'
import { configSerialize } from '@/utils/common'
export default {
  name: 'WebsiteBlackList',
  components: { IconEdit, IconDelete, CreateBlackWebsiteListPop },
  props: {},
  data() {
    return {
      columns: [
      ...
      loading: false,
      dataSource: null,
      createBlackWebsiteListPopVisible: false,
      isEdit: false,
      editId: '',
      selectedRowKeys: []
    }
  ...
  methods: {
    ...
    // 打开新建弹窗
    openCreate() {
      this.createBlackWebsiteListPopVisible = true
    },
    // 打开编辑弹窗
    openEditPop(id) {
      this.editId = id
    ...
    // 编辑弹窗关闭
    handleCreateBlackWebsiteListClose() {

    },
    // 保存成功
    handleCreateBlackWebsiteListSuccess() {
      this.fetch({ pageSize: 10, pageNum: 1 })
    },
    // 删除
    dodelItem(id) {
